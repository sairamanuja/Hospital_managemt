...
export const Admin_doctor_view = () => {
    ...
    const fetchDoctor = async () => {   
        try {
            ...
            setDoctor(response.data.doctor);
             console.log('Doctor details: 445534');
 ...
 useEffect(() => {
    const fetchAppointments = async () => {
        try {
            const response2 = await API.get(`/admin/allAppointment/${id}`);
   ...
   const handleDeleteSlot = async (appointmentId, slotId) => {
            ...
            <div className="w-[80%] h-screen pt-10">
                ...
                <div className="w-full h-full bg-[#f8f9fd] rounded-lg p-6">
                    {doctor ? (
                        <>
                            <div className="flex flex-row">
                                <img src={Neurologist} alt="Doctor" className="w-20 h-20 rounded-full" />
                            </div>
                          ...
                          <div className="pt-10 flex flex-row">
                            ...
                            <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600" onClick={handleAddAppointment}>add appointment</button>
                            <input type="time" placeholder="start time" className="border-2 border-gray-300 rounded-md p-2" onChange={(e) => setStartTime(e.target.value)} />

                            <input type="time" placeholder="end time" className="border-2 border-gray-300 rounded-md p-2" onChange={(e) => setEndTime(e.target.value)} />
                            <input type="date" placeholder="date" className="border-2 border-gray-300 rounded-md p-2" onChange={(e) => setDate(e.target.value)} />
    ...
    {appointments && appointments.map((appointment) => (
        <div key={appointment._id} className="mb-6">
            {appointment.availability.map((schedule) => (
                                ...
                                {schedule.slots.map((slot) => (
                                    <tr key={slot._id}>
                                        ...
                                        <td className="text-center">
                                            <button 
                                                onClick={() => handleDeleteSlot(appointment._id, slot._id)}
